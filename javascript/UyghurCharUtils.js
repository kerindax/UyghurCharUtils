...
].forEach(row => row.map(item => String.fromCharCode(item)).forEach((item, index, list) => charCode[item] = list));

const convertRang = /[\u0622-\u064a\u0675-\u06d5]+/g; //转换范围；不包含哈语的0x0621字母,问号,双引号和Unicode区域的符号
const extendRang = '\\ufb50-\\ufdff\\ufe70-\\ufeff'; //扩展区范围；FB50-FDFF ->区域A    FE70-FEFF -> 区域B
...
class UyghurCharUtils {
  ...
  reverseAscii(source) {
    return source.replace(new RegExp('[^' + extendRang + '\\s]+(\\s+[^' + extendRang + '\\s]+)*', 'g'), (word) => {
  ...
  reverseSubject(str) {// 反转
    return str.replace(/.+/g, function (subject) {//不包含换行符
      return subject.split("").reverse().join("");
    });
  };
  getChar(ch, index) {
    let item = charCode[ch];
    return item ? item[index] : ch;
  };
  extendLa(source) {
    special.forEach(item => {
      source = source.replace(item.link, item.extend);
    });
    return source;
  };
  basicLa(source) {
    special.forEach(item => {
      source = source.replace(item.extend, item.basic);
    });
    return source;
  };
}

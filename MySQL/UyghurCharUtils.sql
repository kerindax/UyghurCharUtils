...
SET @code =_AscW(source);
RETURN (SELECT CASE _index WHEN 0 THEN `0` WHEN 1 THEN `1` WHEN 2 THEN `2` WHEN 3 THEN `3` WHEN 4 THEN `4` END FROM U WHERE `0`=@code OR `1`=@code OR `2`=@code OR `3`=@code OR `4`=@code);
END;

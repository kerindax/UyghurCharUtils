...
namespace Uyghur{
    public class UyghurCharUtils
    {
        ...
        {
            for (int i = 0; i <= U.GetUpperBound(0); i++)
                reg_str += @"\u" + string.Format(U[i, 0].ToString("0x"), "0000");
        }
        /** 基本区   转换   扩展区*/
        public string Basic2Extend(string source)
        {
            ...
            return reg1.Replace(source, word =>
            {
                ...
                if (length > 1)
                {
                    ...
                    returns += Convert.ToChar(ch).ToString();
        ...
        public string Extend2Basic(string source)
        {
            ...
            for (i = 0; i <= source.Length - 1; i++)
            {
                ...
                target += Convert.ToChar(_GetCode(ch, 0)).ToString();
            }
            return target;
        }
        /**反向扩展区   转换   基本区 */
        ...
        private int _GetCode(string source, int index)
        {
            ...
            for (var i = 0; i <= 32; i++)
            {
                if (Convert.ToChar(source[0]) == U[i, 0])
                    return U[i, index];
            }
            return Convert.ToChar(source[0]);
        }

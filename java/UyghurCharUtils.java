    ...
    public String Basic2Extend(String source) {
        return replaceAll(source, convertRang, new replaceCallBack() {
            public String replace(Matcher m) {
                var returns = replaceAll(replaceAll(replaceAll(replaceAll(replaceAll(replaceAll(replaceAll(replaceAll(replaceAll(replaceAll(m.group(0), suffixRang, new replaceCallBack() {
                    public String replace(Matcher m) {
                        return m.group(0) + "  ";
                    }
                }).trim(), "^(\\S)$", new replaceCallBack() {//单字母
                    public String replace(Matcher m) {
                        return getChar(m.group(1), ALONE);
                    }
                }), "^(\\S)(\\S)", new replaceCallBack() {//首字母-没有尾
                    public String replace(Matcher m) {
                        return getChar(m.group(1), ALONE) + m.group(2);
                    }
                }), "(\\S)(\\S)$", new replaceCallBack() {//最后字母-没有尾
                    public String replace(Matcher m) {
                        return m.group(1) + getChar(m.group(2), ALONE);
                    }
                }), "(\\S)(\\S)(\\S)", new replaceCallBack() {//中部字母-没有尾
                    public String replace(Matcher m) {
                        return m.group(1) + getChar(m.group(2), ALONE) + m.group(3);
                    }
                }), "^(\\S)\\s", new replaceCallBack() {//首字母-后部有尾
                    public String replace(Matcher m) {
                        return getChar(m.group(1), HEAD);
                    }
                }), "(\\S)(\\S)\\s", new replaceCallBack() {//中部字母-后部有尾
                    public String replace(Matcher m) {
                        return m.group(1) + getChar(m.group(2), HEAD);
                    }
                }), "\\s(\\S)\\s", new replaceCallBack() {//中部字母-前后有尾
                    public String replace(Matcher m) {
                        return getChar(m.group(1), CENTR);
                    }
                }), "\\s(\\S)$", new replaceCallBack() {//最后字母-前部有尾
                    public String replace(Matcher m) {
                        return getChar(m.group(1), REAR);
                    }
                }), "\\s(\\S)(\\S)", new replaceCallBack() {//中部字母-前部有尾
                    public String replace(Matcher m) {
                        return getChar(m.group(1), REAR) + m.group(2);
                    }
                });

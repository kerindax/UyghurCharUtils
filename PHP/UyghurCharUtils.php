<?php 
   ...
   function Basic2Extend($source){
        ...
        function _ReverseString($source){
            $r = '';
        ...
        function _GetCode($source, $index){
        ...
        function _AscW($source){
            $k = mb_convert_encoding($source, 'UCS-2LE', 'UTF-8');
            $k1 = ord(substr($k, 0, 1));
            $k2 = ord(substr($k, 1, 1));
            return $k2 * 256 + $k1;
            //return hexdec(json_encode($source));
        }
        function _ChrW($number){
